import { updateBoardGame } from "~~/server/db/queries/board-games";
import { UpdateBoardGame } from "~~/server/db/schema";
import { validId } from "~~/server/lib/zod-schema";

export default defineEventHandler(async (event) => {
  const result = await readValidatedBody(event, body => UpdateBoardGame.safeParse(body));
  ...
  try {
    return await updateBoardGame(params.data.id, result.data);
  }
  catch (e) {
    throw createError({
      statusCode: 500,
